...
$serverPath = str_replace("/admin/announcements.php","",$current_url);
...
if( isset($_POST['newsDesc']) && isset($_POST['newsTitle']) ) {
	...
	if( isset($_POST['upd_id']) && ( (int)$_POST['upd_id'] != 0 ) ) {
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,'announcements',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,date('Y-m-d H:i'),'announcements',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
	...
	if($res) {
       ...
       if( $New == 1 ) {
    	   $_SESSION['msg'] = 'Announcement added successfully...';
		    ...
		    <div class="General_r">
		      <table border="0">
		      	<tr>
		      		...
		      		<?php if($pdfName != '') { echo $pdfName; echo '<span class="deleteText"><a href="announcements.php?id='.$id.'&del=1">DELETE</a></span>'; } ?> 
		      	</tr>
		      </table>
		    </div>
		    <div class="clear"></div>
