...
$serverPath = str_replace("/admin/events.php","",$current_url);
...
if( isset($_POST['newsDesc']) && isset($_POST['newsTitle']) ) {
	...
	if( isset($_POST['upd_id']) && ( (int)$_POST['upd_id'] != 0 ) ) {
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,'events',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,date('Y-m-d H:i'),'events',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
		    ...
		    <div class="General_r">
		      <table border="0">
		      	<tr>
		      		...
		      		<?php if($pdfName != '') { echo $pdfName; echo '<span class="deleteText"><a href="blogs.php?id='.$id.'&del=1">DELETE</a></span>'; } ?> 
		      	</tr>
		      </table>
		    </div>
		    <div class="clear"></div>
