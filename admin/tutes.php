...
$serverPath = str_replace("/admin/tutes.php","",$current_url);
...
if( isset($_POST['newsDesc']) && isset($_POST['newsTitle']) ) {
	...
	if( isset($_POST['upd_id']) && ( (int)$_POST['upd_id'] != 0 ) ) {
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,'blogs',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
		...
		$fieldValues = array($newsTitle,$href,$shortdescr,$newsDesc,date('Y-m-d H:i'),'blogs',$pdfName,$cp_mt,$cp_mk,$cp_md,$pubDate);
	...
	if($res) {
	   $_SESSION['msg'] = 'Tute saved successfully...';
       if( $New == 1 ) {
    	   $_SESSION['msg'] = 'Tute added successfully...';
		    ...
		    <div class="General_r">
		      <table border="0">
		      	<tr>
		      		...
		      		<?php if($pdfName != '') { echo $pdfName; echo '<span class="deleteText"><a href="tutes.php?id='.$id.'&del=1">DELETE</a></span>'; } ?> 
		      	</tr>
		      </table>
		    </div>
		    <div class="clear"></div>
